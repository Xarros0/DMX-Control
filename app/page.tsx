...
export default function Home() {
  ...
  const [groups, setGroups] = useState(() => {
    ...
    return [
      ...
      },
      {
        ...
        name: "Group 4",
        color: "amber",
        lights: Array.from({ length: 2 }).map((_, i) => ({
          id: `4-${i + 1}`,
          name: `Light ${i + 12}`,
          color: "#ffffff",
          on: true,
      ...
      prev.map((g) => {
        ...
        return {
          ...
          lights: [
            ...g.lights,
            {
              id: `${groupId}-${nextIndex}`,
              name: `Light ${Math.floor(Math.random() * 1000)}`,
              color: "#ffffff",
              on: true,
            },
          ],
  ...
  function deleteLight(groupId, lightId) {
    setGroups((prev) =>
      prev.map((g) =>
        g.id === groupId
          ? { ...g, lights: g.lights.filter((l) => l.id !== lightId) }
          : g
      )
    );
            ...
            <main className="col-span-6">
              <div className="flex items-start gap-4">
                <div className="flex-1 space-y-4">
                  <div className="w-20">
                    <div className="shadow p-2 rounded bg-white text-center text-sm">
                      Master
                    </div>
                  </div>
                  {groups.map((group) => (
                    <div
                      ...
                      className={`rounded border-2 p-3
                        ${group.color === "indigo" ? "border-indigo-500 bg-indigo-50" : ""}
                        ${group.color === "green" ? "border-green-400 bg-green-50" : ""}
                        ${group.color === "red" ? "border-red-400 bg-red-50" : ""}
                        ${group.color === "amber" ? "border-amber-300 bg-amber-50" : ""}
                        ${group.color === "blue" ? "border-blue-400 bg-blue-50" : ""}
                        ${group.color === "purple" ? "border-purple-400 bg-purple-50" : ""}
                        ${group.color === "pink" ? "border-pink-400 bg-pink-50" : ""}
                        ${group.color === "teal" ? "border-teal-400 bg-teal-50" : ""}
                      `}
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                          <Button
                            onClick={() => addLightToGroup(group.id)}
                      ...
                      <div className="mt-3 grid grid-cols-4 gap-3">
                        {group.lights.map((light) => (
                          <div
                            key={light.id}
                            className={`p-2 border rounded bg-white shadow-sm relative ${
                          ...
                          >
                            ...
                            <div className="mt-2 flex gap-2">
                              <button
                                onClick={() => deleteLight(group.id, light.id)}
                                className="text-sm px-2 py-1 rounded bg-red-50"
                              >
                                Delete -
                              </button>
                            </div>
                  ...
                  <div className="flex gap-4 mt-4">
                    ...
                    <Button
                      onClick={() => console.log("delete light (global)")}
                      className=""
                    >
                      Delete Light -
                    </Button>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center justify-center">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (selectedLight) {
                            const l = allLights.find(
                              (x) => x.id === selectedLight
                            );
                            if (l)
                              updateLight(l.groupId, l.id, {
                                color: e.target.value,
                              });
                          }
                        }}
                        className="w-40 h-40 border-4 border-white shadow"
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="text-sm text-gray-500 px-2">X-axis</div>
                        <input
                          className="px-2"
                          type="range"
                          min={0}
                      ...
                      <div>
                        <div className="text-sm text-gray-500 px-2">Y-axis</div>
                        <input
                          className="px-2"
                          type="range"
                          min={0}
          ...
          <div className="max-w-[1200px] mx-auto mt-6">
            <Card>
              <div className="text-sm text-gray-900 px-2">
                Selected light: {selectedLight || "—"}. Master:{" "}
                {masterBrightness}%
              </div>
              <pre className="mt-2 text-sm text-gray-900 bg-gray-100 p-2 px-2">
                {JSON.stringify(
                  { groups, masterBrightness, selectedColor, xy },
                  null,
                  2
                )}
              </pre>
